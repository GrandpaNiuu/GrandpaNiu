...
| 美团 | 本地生活 | MITM, Remote, Rewrite, Rule, Script | stable, stable-plus, lite, full | 通过 | 高 | 2026-06-01 | manual_test_log.md / 用户确认 | Stable 第一轮通过；后续敏感链路变更仍需复测 | 回滚 Rewrite、Rules、Scripts 中对应源头后重建 | 用户确认，不是助手亲测；大改后仍需复测 |
...
| 高德地图 | 地图 | MITM, Remote, Rewrite, Rule, Script | stable, stable-plus, lite, full | 未测 | 高 | 未记录 | 无真实记录 | 未测或高风险，需人工复核 | 回滚 Rewrite、Rules、Scripts 中对应源头后重建 | 覆盖存在不等于测试通过；未测必须保持未测 |
