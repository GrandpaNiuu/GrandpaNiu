...
- 生成时间：2026-06-01 00:18:15 +0800
- 状态模式：GitHub API 真实状态模式

| Workflow | 用途 | 触发方式 | 最近运行时间 | 最近状态 | 结论 | 失败 Job | 失败 Step | 对应 commit | 失败时优先排查 |
|---|---|---|---|---|---|---|---|---|---|
| Module Factory Build | 构建 Release 并同步 Root | 手动 / push | 2026-05-31T16:11:02Z | completed / failure | 需要检查 | build | Commit factory output | e01b0a96fbb1 | build_module.py、factory_finalize.py、profile、sources、Root/Release diff |
| Daily Module Update | 每日日期、结构、链接和验证检查 | 手动 / 定时 / push | 2026-05-31T01:39:23Z | completed / failure | 需要检查 | update | Commit daily update | fb96b93a3763 | 核心标记、远程链接、validate_repository.py 输出 |
...
| Repository Health Check | 仓库治理健康检查 | 手动 / 定时 / push | 2026-05-31T16:18:12Z | in_progress / running | 需要检查 | 无 | 无 | ad4d3b236b62 | 缺失治理文件、README 链接、重复脚本、重复 MITM |
