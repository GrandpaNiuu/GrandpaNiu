...
| 百度地图 | MITM, Rule, URL Rewrite | 局部覆盖 | 高 | Rewrite/Sources/MITM.conf<br>Rewrite/Sources/URL-Rewrite.conf<br>Rules/direct.list<br>Rules/protect-login.list<br>Rules/qingrex-miniapp-app-ad.list<br>Rules/reject.list<br>Rules/web-ads.list | 自动扫描已覆盖 | 首页、搜索、详情页、核心流程由用户反馈或 Issue 观察，不作为自动门禁。 | 高风险项必须保留保护规则和回滚路径 |
