...
本文件定义维护 GrandpaNiu 仓库时的强制边界。目标是让仓库保持可构建、可验证、可回滚，而不是盲目扩大规则、脚本或 MITM 覆盖。

## 总原则

1. **源头优先**：先改 `Rules/`、`Scripts/`、`Rewrite/Sources/`、`Rewrite/Profiles/`、`Rewrite/Remotes/`，再构建 `Release/` 和根目录模块。
2. **正式版保守**：默认正式入口只保留低误伤、可长期使用、可回滚内容。
3. **增强内容先测试**：增强内容先进入测试配置或候选区，不得整体合并进正式入口。
4. **低风险定位优先**：排查时优先使用低 MITM、低脚本、低风险方式定位问题。
5. **全量覆盖只做排查**：全量覆盖配置不适合长期启用，不得作为默认发布目标。
6. **敏感链路默认保护**：登录、支付、验证码、银行、微信、支付宝、Cookie、Token、会员权益、账号状态默认不拦截、不改写。
7. **一次变更只处理一个风险单元**：一个 App、一类 CDN、一组 HTTPDNS、一个低风险脚本融合项，或一批同源低风险广告域。

## 任务分级

| 级别 | 任务类型 | 允许修改 | 禁止修改 | 验收重点 |
|---|---|---|---|---|
| P0 | 构建失败、验证失败、报告阻断、Root/Release 不一致 | 构建脚本、验证脚本、报告生成脚本、对应源头文件 | 扩大正式入口 MITM、批量删除 REJECT、启用未知脚本 | `validate_repository.py` 通过，blocking stale 为 0 |
| P1 | 治理文档、报告闭环、workflow 状态、自动化验证计划 | `docs/`、`reports/`、报告生成脚本、README 入口 | 修改广告规则语义、扩大 hostname、伪造自动化验证 | 文档一致，报告可追溯 |
| P2 | 单项 App 修复、增强测试、候选源评估 | 单个 App 对应源头、测试配置或 pending 配置、审计报告 | 整体合并测试配置，批量迁移 MITM | 单项可回滚，有测试计划 |
...
不满足以上条件时，只能进入测试配置、pending 或 manual-review，不能进入正式入口。
...
| 影响版本 | formal / test / lite / full |
| 影响 App / 服务 | 指出可能受影响的 App 或服务 |
| 回滚动作 | 删除、恢复、移出 profile、补 DIRECT 或恢复旧脚本入口 |
| 重建命令 | 至少包含 build、sync root、build release variants、validate_repository |
